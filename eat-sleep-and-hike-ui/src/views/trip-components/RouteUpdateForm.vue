<template>
  ...
  <div class="container bg-white p-4 my-5 rounded shadow-sm" style="max-width: 700px;">
    ...
    <!-- Submit -->
    <button class="btn btn-success w-100" @click="submitForm">Update Route</button>
  </div>
</template>
  ...
  methods: {
    ...
    submitForm() {
      ...
      try{
        ...
        routeService.update(this.form)
      }catch (e){
        alert("An error occurred when updating the route")
        console.error(e)
        return
      }
      console.log("Route is updated:", this.form);
      this.$router.push(this.path);
      alert("Route created!");
    },

    async fetchData(){
      const data = await routeService.getById(this.$route.params.id)
      this.form.id = data.id
      this.form.title = data.name
      this.form.description = data.description
/*      this.form.points = data.routePointDtos.map(rp => {
        return {
          id: rp.id,
      ...
      this.form.points = await routeService.getRoutePointByRouteId(data.id)
      console.log(this.form.points)
      this.form.countries = data.countryDtos.map(c => {
        return {
          id: c.id,
          name: c.name
        }
